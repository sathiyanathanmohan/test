...
function manager1()
{
	...
	var url = "rest/webservices/getallemployeemanager?designation=associate";
		...
		$.ajax({
			...
			success: function(data) {
				...
				for ( var i in object1) {
					...
					designation=object1[i].designation;
	...
	$.ajax({
		...
		success: function (data) {
			...
			if(data.status==0){
				...
				if(designation=='project_manager')
				{
					var splits = emp_code.split(/(\d)/);
					if(splits[0]!='')
					{
						if(splits[0].toUpperCase()=='BNG')
						{
							branch_code='bng062';

						}
						else if(splits[0].toUpperCase()=='CHN')
						{
							branch_code='chn001';
						}
					}
					var url3 = "rest/webservices/managermail?branch="+branch_code+"&manager_code="+user2+"&emp_code="+e_code+"&project_code="+pro_code;
...
function esave(){
	var ee_code=$('#eempid').val();
	document.getElementById("epro_code").value = project_code;
	var url = "rest/webservices/updateallocation";
	var pro_code=$('#epro_code').val();
	alert('procode'+pro_code);
...
//						alert('bangalore');
				...
				var url3 = "rest/webservices/managermail?branch="+branch_code+"&manager_code="+user2+"&emp_code="+ee_code+"&project_code="+pro_code;
